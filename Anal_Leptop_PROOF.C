  ...
  for(int i=0;i<3;i++)
    {
      char namein[1000];
      char titlein[1000];
      sprintf(namein,"%s",nbjetR_names[i]);
      sprintf(titlein,"%s",nbjetR_title[i]);
  ...
  for(int i=0;i<26;i++)
    {
      char namein[1000];
      char titlein[1000];
      sprintf(namein,"%s",new_var_names[i]);
      sprintf(titlein,"%s",new_var_title[i]);
...
Bool_t Anal_Leptop_PROOF::Process(Long64_t entry)
{
  ...
  if(isMC)
    {
      int top_blep[2]={-1,-1};
      int antitop_blep[2]={-1,-1};
      int top_igen=-1;
      int antitop_igen=-1;
      
      for(int igen=0; igen<ngenparticles; igen++)
	{
	  if(genpartpdg[igen]==6 && genpartgrmompdg[igen]==2212) {top_igen=igen;}
	  if(genpartpdg[igen]==-6 && genpartgrmompdg[igen]==2212) {antitop_igen=igen;}
	  if(!(genpartstatus[igen]==23 || genpartstatus[igen]==1)) continue;
	  if(!(genpartfromhard[igen])) continue;
	  if( (genpartpdg[igen]==11||genpartpdg[igen]==13||genpartpdg[igen]==15) && genpartmompdg[igen]==-24 && genpartgrmompdg[igen]==-6 ) { antitop_blep[1]=igen; }  //lep index at 1 and b quark index at 0
	  if(genpartpdg[igen]==-5 && genpartmompdg[igen]==-6) { antitop_blep[0]=igen; }
	  if( (genpartpdg[igen]==-11||genpartpdg[igen]==-13||genpartpdg[igen]==-15) && genpartmompdg[igen]==24 && genpartgrmompdg[igen]==6 ) {  top_blep[1]=igen; }
	  if(genpartpdg[igen]==5 && genpartmompdg[igen]== 6) {  top_blep[0]=igen; }
	  ...
	  if(abs(genpartpdg[igen])==11)
	    {
	      ...
	      genelectronpdg[ngenelc] = genpartpdg[igen];
	      ngenelc++;
	    }
	  if(abs(genpartpdg[igen])==13)
	    {
	      ...
	      genmuonpdg[ngenmu] = genpartpdg[igen];
	      ngenmu++;
	    }
    ...
    TLorentzVector genlep1,genlep2,leadgenb,leadgenjetak4,leadgenjetak8;
    leadgenb.SetPtEtaPhiM(0,0,0,0);
    leadgenjetak4.SetPtEtaPhiM(0,0,0,0);
    leadgenjetak8.SetPtEtaPhiM(0,0,0,0);
    genlep1.SetPtEtaPhiM(0,0,0,0);
    genlep2.SetPtEtaPhiM(0,0,0,0);
    
    int genl1,genl2,genbmax,igenjetmax;
    genl1 = genl2 = genbmax = igenjetmax = -10;
    
    float bptmax=-100;
    float jetptmax=-100;
    float lep1pt=-100;
    float lep2pt=-100;
    //For leading  gen b quark
    for(int ib=0; ib<ngenbs; ib++)
      {
	if(genbpt[ib]>bptmax)
	  {
	    bptmax=genbpt[ib];
    ...
    if(genbmax > -1)      leadgenb.SetPtEtaPhiM(genbpt[genbmax],genbeta[genbmax],genbphi[genbmax],genbm[genbmax]);
    // For leading gen  ak4 jet, ht sum and pt sum 
    float htsum=0;
    TLorentzVector ptsumak4(0,0,0,0);
    TLorentzVector ak4tmp;
    for(int ijet=0; ijet<ngenjetAK4; ijet++)
      {
	if(genjetAK4pt[ijet] > jetptmax)
	  {
	    jetptmax = genjetAK4pt[ijet];
	    igenjetmax = ijet;
	  }
	if(genjetAK4pt[ijet]>30.0)
	  {
	    htsum += genjetAK4pt[ijet];
	    ak4tmp.SetPtEtaPhiM(genjetAK4pt[ijet],genjetAK4eta[ijet],genjetAK4phi[ijet],genjetAK4mass[ijet]);
	    ptsumak4 = ptsumak4 + ak4tmp;
	  }
      }

    if(igenjetmax > -1)     leadgenjetak4.SetPtEtaPhiM(genjetAK4pt[igenjetmax],genjetAK4eta[igenjetmax],genjetAK4phi[igenjetmax],genjetAK4mass[igenjetmax]);
    //FOr leading gen ak8 jet
    igenjetmax = -10;
    jetptmax=-100;
    
    for(int ijet=0; ijet<ngenjetAK8; ijet++)
      {
	if(genjetAK8pt[ijet] > jetptmax)
	  {
	    jetptmax = genjetAK8pt[ijet];
	    igenjetmax = ijet;
	  }
      }
    
    if(igenjetmax > -1)     leadgenjetak8.SetPtEtaPhiM(genjetAK8pt[igenjetmax],genjetAK8eta[igenjetmax],genjetAK8phi[igenjetmax],genjetAK8mass[igenjetmax]);
    //gProofServ->SendAsynMessage("Check 1");
    // For leading and second leading gen lep
    int genlepch1=0;   int genlepch2=0;
    for(int iel=0;iel<ngenelectrons;iel++)
      {
	if(genelectronpt[iel] > lep1pt)
	  {
	    lep2pt=lep1pt;
	    genl2=genl1;
	    genlepch2=genlepch1;
	    lep1pt=genelectronpt[iel];
	    genl1=iel;
	    genlepch1=genelectronpdg[iel];
	  }
	else if(genelectronpt[iel] > lep2pt)
	  {
	    lep2pt=genelectronpt[iel];
	    genl2=iel;
	    genlepch2=genelectronpdg[iel];
	  }
      }
    
    for(int imu=0;imu<ngenmuons;imu++)
      {
	if(genmuonpt[imu] > lep1pt)
	  {
	    lep2pt=lep1pt;
	    genl2=genl1;
	    genlepch2=genlepch1;
	    lep1pt=genmuonpt[imu];
	    genl1=imu + ngenelectrons;
	    genlepch1=genmuonpdg[imu];
	  }
	else if(genmuonpt[imu] > lep2pt)
	  {
	    lep2pt=genmuonpt[imu];
	    genl2=imu + ngenelectrons;
	    genlepch2=genmuonpdg[imu];
	  }
      }
  
    if(genl1<ngenelectrons && genl1>-1)                genlep1.SetPtEtaPhiM(genelectronpt[genl1],genelectroneta[genl1],genelectronphi[genl1],genelectronm[genl1]);
    else if(genl1<ngenmuons + ngenelectrons && genl1>-1)    genlep1.SetPtEtaPhiM(genmuonpt[genl1-ngenelectrons],genmuoneta[genl1-ngenelectrons],genmuonphi[genl1-ngenelectrons],genmuonm[genl1-ngenelectrons]);
    
    if(genl2<ngenelectrons && genl2>-1)                  genlep2.SetPtEtaPhiM(genelectronpt[genl2],genelectroneta[genl2],genelectronphi[genl2],genelectronm[genl2]);
    else if(genl2<ngenmuons + ngenelectrons && genl1>-1)      genlep2.SetPtEtaPhiM(genmuonpt[genl2-ngenelectrons],genmuoneta[genl2-ngenelectrons],genmuonphi[genl2-ngenelectrons],genmuonm[genl2-ngenelectrons]);

    // gProofServ->SendAsynMessage("Check 2");
    //if(GENMET>0 && genlep1.Pt() >0 && genlep2.Pt() >0 && leadgenjetak4.Pt() >0 && leadgenb.Pt()>0 && leadgenjetak8.Pt()>300)
	...
	top_dp[idp] = igen;
	if(abs(genpartpdg[igen])>=1 && abs(genpartpdg[igen])<5 && abs(genpartmompdg[igen])==24 && abs(genpartgrmompdg[igen])==6 ) {  top_dqp[ndq] = igen;  ndq++; }
	if(abs(genpartpdg[igen])>=11 && abs(genpartpdg[igen])<=16 && abs(genpartmompdg[igen])==24 && abs(genpartgrmompdg[igen])==6 ) {  top_dlp[ndl] = igen;  ndl++; }
	...
	TLorentzVector nearestb;
	int inearestb=-10;
	float delRmintmp=100;
	for(int ib=0; ib<ngenbs; ib++)
	  {
	    float tmp;
	    tmp = delta2R(genlep1.Eta(),genlep1.Phi(),genbeta[ib],genbphi[ib]);
	    if(tmp<delRmintmp)
	      {
		delRmintmp=tmp;
		inearestb=ib;
	...
	for(int ib=0; ib<ngenbs; ib++)
	  {
	    float tmp;
	    tmp = delta2R(genlep2.Eta(),genlep2.Phi(),genbeta[ib],genbphi[ib]);
	    if(tmp<delRmintmp)
	      {
		delRmintmp=tmp;
		inearestb=ib;
